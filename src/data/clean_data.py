...
def get_joined_fin_dataframe():
    current_dir = op.dirname(__file__)
    fin_recipients_path = op.join(current_dir, '..', '..', 'data', 'raw',
    ...
    fin_recipients_df = fin_recipients_df.drop_duplicates(subset=('year_reported',
                                                                  'recipient_name'))

    fin_support_df = pd.DataFrame()
    for year in YEARS:
        filename = op.join(current_dir, '..', '..', 'data', 'raw',
